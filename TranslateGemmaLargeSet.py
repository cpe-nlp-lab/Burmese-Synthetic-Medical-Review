...
class DatasetTranslator:
    ...
    def is_rate_limit_error(self, error):
        ...
        rate_limit_indicators = [
            ...
            'rate_limit_exceeded', 'requests per', 'limit exceeded', 'throttle',
            'usage limit', 'daily limit', 'monthly limit', 'per minute', 'per hour'
        ]
    ...
    def wait_with_progress(self, wait_seconds, reason="Rate limit"):
        ...
        self.logger.info(f"{reason} detected. Waiting {wait_seconds} seconds ({wait_seconds/60:.1f} minutes)")
        for remaining in tqdm(range(wait_seconds, 0, -1), desc=f"Waiting ({reason})", unit="sec"):
            time.sleep(1)
            if remaining % 60 == 0:
                self.logger.info(f"Still waiting... {remaining//60} minutes remaining")
        self.logger.info(f"Wait period completed. Resuming translation...")
        ...
        while actual_attempt < max_retries:
            try:
                response = self.client.models.generate_content(
                    ...
                    contents="Please translate this text from English to Burmese language. "
                             "Do not add any additional information or context. Just provide the translation. [Text]: "
                             + str(text)
                )
                ...
                if response.text and response.text.strip():
                    ...
                    self.logger.warning(f"Empty response for text: {str(text)[:50]}...")
                    return "Translation failed - empty response"
                    
            except Exception as e:
                self.logger.warning(f"Attempt {actual_attempt + 1} failed for text: {str(text)[:50]}... Error: {e}")
    ...
    def process_in_chunks(self, df, chunk_size=100, checkpoint_interval=50):
        ...
        total_rows = len(df)
        checkpoint = self.load_checkpoint("translation_checkpoint_latest.json")
        ...
        for i, text in enumerate(tqdm(texts_to_process, desc="Translating", initial=start_index, total=total_rows)):
            ...
            if (current_index + 1) % checkpoint_interval == 0:
                self.save_checkpoint("translation_checkpoint_latest.json", current_index + 1, translated_texts)
            ...
            if hasattr(self, '_consecutive_failures'):
                self._consecutive_failures += 1 if 'failed' in translation else 0
            else:
                self._consecutive_failures = 1 if 'failed' in translation else 0
        ...
        self.save_checkpoint("translation_checkpoint_latest.json", total_rows, translated_texts)
    ...
    def save_intermediate_results(self, df, translated_texts, completed_count):
        ...
        try:
            ...
            filename = f'Burmese_Medication_Gemma3-27b-it_partial_{completed_count}_{timestamp}.csv'
    ...
    def translate_dataset(self, csv_file, chunk_size=100, checkpoint_interval=50):
        ...
        try:
            ...
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            output_filename = f'Burmese_Medication_Gemma3-27b-it_final_{timestamp}.csv'
            ...
            if os.path.exists("translation_checkpoint_latest.json"):
                os.rename("translation_checkpoint_latest.json", f"translation_checkpoint_completed_{timestamp}.json")
...
def main():
    ...
    try:
        ...
        print(f" Resume by running the script again - it will continue from the last checkpoint.")
        
    except Exception as e:
        print(f"\n Error occurred: {e}")
        print(f" Check the log file and checkpoint for recovery options.")
